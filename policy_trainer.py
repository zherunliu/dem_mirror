...
class PolicyTrainer:
    def __init__(
        ...
        self._rollout_freq, self._rollout_batch_size, self._rollout_length = (
            rollout_setting
        )
    ...
    def train(self) -> Dict[str, float]:
        ...
        for e in range(1, self._epoch + 1):
            ...
            for it in pbar:
                if num_timesteps % self._rollout_freq == 0:
                    init_obss = (
                        self.real_buffer.sample(self._rollout_batch_size)[
                            "observations"
                        ]
                        .cpu()
                        .numpy()
                    )
                    rollout_transitions, rollout_info = self.policy.rollout(
                        init_obss, self._rollout_length
                    )
                    self.fake_buffer.add_batch(**rollout_transitions)
                    self.logger.log(
                        "num rollout transitions: {}, reward mean: {:.4f}".format(
                            rollout_info["num_transitions"], rollout_info["reward_mean"]
                        )
                    )
                ...
                if (
                    0 < self._dynamics_update_freq
                    and (num_timesteps + 1) % self._dynamics_update_freq == 0
                ):
            ...
            ), np.std(eval_info["eval/episode_reward"])
        ...
        torch.save(
            self.policy.state_dict(), os.path.join(self.logger.model_dir, "policy.pth")
        )
        self.policy.dynamics.save(self.logger.model_dir)

        return {"mean_performance": np.mean(last_10_performance)}
