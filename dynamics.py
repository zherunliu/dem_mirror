...
class EnsembleDynamics(object):
    ...
    def step(
        ...
        ensemble_samples = (mean + np.random.normal(size=mean.shape) * std).astype(
            np.float32
        )
        ...
        # compute uncertainty for aleatoric and epistemic
        if self._penalty_coef:
            penalty_alea = np.amax(np.linalg.norm(std, axis=2), axis=0)
            next_obses_mean = mean[..., :-1]
            next_obs_mean = np.mean(next_obses_mean, axis=0)
            diff = next_obses_mean - next_obs_mean
            penalty_pair = np.amax(np.linalg.norm(diff, axis=2), axis=0)
            penalty_ens = np.sqrt(next_obses_mean.var(0).mean(1))
            penalty_alea = np.expand_dims(penalty_alea, 1).astype(np.float32)
            penalty_pair = np.expand_dims(penalty_pair, 1).astype(np.float32)
            penalty_ens = np.expand_dims(penalty_ens, 1).astype(np.float32)
            assert penalty_alea.shape == reward.shape
            # offline
            if self._uncertainty_mode == "aleatoric":
                reward = reward - self._penalty_coef * penalty_alea
            # online
            elif self._uncertainty_mode == "epistemic":
                reward = reward + self._penalty_coef * (penalty_alea + penalty_ens)

        if not pred:
            return penalty_ens + penalty_alea

        return next_obs, reward, terminal, info

    @torch.no_grad()
    def sample_next_obss(
        self, obs: torch.Tensor, action: torch.Tensor, num_samples: int
    ) -> torch.Tensor:
        ...
        samples = torch.stack(
            [mean + torch.randn_like(std) * std for i in range(num_samples)], 0
        )
        next_obss = samples[..., :-1]
        return next_obss
    ...
    def train(
        ...
        train_inputs, train_targets = (
            inputs[train_splits.indices],
            targets[train_splits.indices],
        )
        holdout_inputs, holdout_targets = (
            inputs[holdout_splits.indices],
            targets[holdout_splits.indices],
        )
        ...
        data_idxes = np.random.randint(
            train_size, size=[self.model.num_ensemble, train_size]
        )
        ...
        while True:
            ...
            if (cnt >= max_epochs_since_update) or (
                max_epochs and (epoch >= max_epochs)
            ):
                break
    ...
    def load(self, load_path: str) -> None:
        self.model.load_state_dict(
            torch.load(
                os.path.join(load_path, "dynamics.pth"), map_location=self.model.device
            )
        )
